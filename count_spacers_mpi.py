...
This is a MPI version working on clusters across multiple nodes.
'''
...
def count_spacers_mpi(fastq_file):
    out_file = os.path.splitext(fastq_file)[0]+'_Count.csv'
    count_spacers('lib_brie_sgRNA.csv', fastq_file, out_file, True)

if __name__ == '__main__':
    ''' sbatch runmpi.slurm
    ## runmpi.slurm in Atom:
    #!/usr/bin/bash
    ...
    executor.map(count_spacers_mpi, files)
    executor.shutdown()
